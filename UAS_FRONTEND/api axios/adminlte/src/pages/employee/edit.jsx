...
const EditEmployee = () => {
  const { nik } = useParams(); // Mendapatkan ID karyawan dari URL
  ...
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm(); // Menginisialisasi useForm

  useEffect(() => {
    // Mengambil data karyawan yang akan diedit dan mengatur nilai-nilai awal formulir
    axios
      .get(`https://63929998b750c8d178e16014.mockapi.io/api/employee/${nik}`)
      .then((response) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        Swal.fire({
          icon: "success",
          title: "Success",
          text: "Data Edit added successfully!",
        }).then(() => {
          Navigate("/users");
        });
